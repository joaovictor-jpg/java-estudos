...
public class EmailService {
    ...
    public void enviarEmailComSenhaGeradaAleatoriamente(Usuario usuario) {
        String assunto = "Senha gerada";
        String conteudo = gerarConteudoEmailParaSenha("Olá [[name]],<br>"
                        ...
                        + "Obrigado,<br>",
                usuario.getNome(), usuario.getUsername(), usuario.getPassword(), URL_SITE + "/home"
        );
    ...
    private String gerarConteudoEmailParaSenha(String template, String nome, String email, String senha, String url) {
        return template.replace("[[name]]", nome).replace("[[email]]", email).replace("[[senha]]", senha).replace("[[URL]]", url);
    }
}
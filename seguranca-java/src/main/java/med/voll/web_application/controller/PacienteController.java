...
public class PacienteController {
    ...
    public String carregarPaginaListagem(@PageableDefault Pageable paginacao, Model model) {
        var pacienteCadastrados = service.listar(paginacao);
        model.addAttribute("pacientes", pacienteCadastrados);
    ...
    @PostMapping
    public String cadastro(@Valid @ModelAttribute("dados") DadosCadastroPaciente dados, BindingResult result, Model model) {
        if (result.hasErrors()) {
